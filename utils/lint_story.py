...
for file in BASE_DIR.glob("characters/*.json"):
    with open(file) as f:
        ...
        if isinstance(c, dict):
            character_info[c["name"]] = {"species": None, "role": None}
            location_data[c["name"]] = {"regions": []}
        elif isinstance(c, list):
            for char in c:
                if isinstance(char, dict) and "name" in char:
                    character_info[char["name"]] = {"species": None, "role": None}
                    location_data[char["name"]] = {"regions": []}
...
for event in load_json(TIMELINE_PATH):
    ...
    if isinstance(characters, list) and len(characters) > 0:
        ...
        if isinstance(flora_data, dict):
            ...
            species_data_entry = next(
                (
                    ...
                    if isinstance(item, dict) and item.get("name") == char_species_name
                ),
                {},
            )
        else:
        ...
        print(
            f"⚠️ Warning: Missing character data in event '{event.get('id', 'unknown')}': Characters list is empty or not a list"
        )
        continue
    ...
    for char_name in character_info:
        if isinstance(character_info[char_name], dict):
            # Species assignment
            if isinstance(flora_data, dict):
                character_info[char_name]["species"] = flora_data.get(
                    char_name, {}
                ).get("species")
            elif isinstance(flora_data, list):
                species_obj = next(
                    (
                        item
                        for item in flora_data
                        if isinstance(item, dict) and item.get("name") == char_name
                    ),
                    {},
                )
    ...
    for spec_name in species:
        ...
        if (
            isinstance(flora_data_for_species, dict)
            and spec_name in flora_data_for_species
        ):
            species[spec_name]["evolution_chain"] = flora_data_for_species[
                spec_name
            ].get("evolution_chain")
            ...
            species_obj = next(
                (
                    item
                    for item in flora_data_for_species
                    if isinstance(item, dict) and item.get("name") == spec_name
                ),
                {},
            )
    ...
    for name, char_info in character_info.items():
        ...
        if "role" not in char_info or char_info["role"] is None:
            print(
                f"⚠️ Warning: Missing location/role information for character '{name}'"
            )
