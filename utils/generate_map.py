    ...
    with open(path, 'r', encoding='utf-8') as f:
        ...
        try:
            ...
            if required:
                raise ValueError(msg)
            return None
        if geojson and (not isinstance(data, dict) or data.get('type') != 'FeatureCollection'):
...
regions_geojson = load_json_file('cartography/regions.geojson', required=False, geojson=True)
