...
| **species declared** | A `species` is one of those in `database/species.json`. The same gap `culture` had, one field over: 12 free-text values across 52 characters, near-duplicate pairs among them, and nothing checking any of it. Characters only -- fauna and flora answer "what is this" through `clade` and `base_species`. |
| **a rebirth points backwards** | `reincarnation_of` resolves, is not itself, and names a character in an *earlier* epoch. A second life beginning before the first is the same error as an event edge running backwards through time. |
| **a myth does not attend things** | No event names a `mythology_` id among its participants, witnesses or actors. A mythology entity can be *about* an event -- that is what `relations` is for -- but cannot be present at one. Owlman and the Ammonite Man were both characters filed as mythology, and both were caught by a reader rather than by the gate. |
| **the prose names the cast** | Nobody named in an event's `title` or `summary` is missing from its `participants`, `witnesses` or `actors`, aliases included. `event_shadow_pact` said Varunesh summons "the Asura princess Manjalaya" and then listed two participants, neither of them her -- she was the only character in canon present at nothing, so the memory map drew her isolated from data that was wrong. `causes` and `outcomes` are deliberately not scanned: a cause names what led here, which is routinely somebody dead. |
| **a drawn place is dated** | A place or settlement carrying `coordinates`, an `extent`, a `path` **or a `within`** names its `epochs`. Silence means every era, which is right for a crocodile and wrong for a city: six eras drew the same map, with Harappa standing among the Vanaras. Places canon has not located may stay undated. |
| **a place may inherit a position** | `within` names what contains a place -- another place, a region, a settlement or a field map -- and the atlas resolves a position from it, drawn hollow rather than filled. One hop only: a chain would let an unplaced place borrow from another unplaced place. |
...
| **a material is gatherable where its source lives** | A material's `found_in` names no biome that none of its `won_from` species can reach. `material_ammonite_shell` was gathered in `coast` and `sea` while both its ammonites live in `lava_field` and `mountains` — **no biome in common**, so the shell was takeable in every biome the animal cannot survive in and none of the ones it can. Twenty-five materials disagreed with their own sources; nothing saw it, because each file is internally consistent and the contradiction lives only *between* them. A **subset** check, not equality: fewer biomes than the species reach is under-supply and harmless, more is the bug. Three materials that genuinely travel without their source — leviathan bone, oyster shell, salt crust — are exempt by id in the linter and say why in their own `notes`. |
| **a map's vehicles can float on it** | Every id in a field map's `vehicles` is a vehicle (the schema's `^vehicle_` pattern catches the prefix; the lint catches an id that has the prefix and lives in the wrong folder), and at least one of that vehicle's `crosses` biomes is in the map's `seed_biomes`. The failure it prevents is silent: a boat listed on a map with no water it can travel on is valid in both files and would simply never be offered by the game. Only Lothal carries the field, for the dugout. |
| **renewal vocabulary is pinned** | `renewal_rates.json` and the `renews` enum in `material.schema.json` declare the same four values, checked both ways like the material-class pin above. `renews` is an ordering and never a duration: canon says salt-crust returns faster than sandalwood, and how many days that is belongs to the game. |
