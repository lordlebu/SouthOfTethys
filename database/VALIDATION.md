...
| **a drawn place is dated** | A place carrying `coordinates`, an `extent` or a `path` names its `epochs`. Silence means every era, which is right for a crocodile and wrong for a city: six eras drew the same map, with Harappa standing among the Vanaras. Places canon has not located may stay undated. |
